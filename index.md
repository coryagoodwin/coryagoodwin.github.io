...
The concept of releasing early and often is something that was very popularized through Linux's successful usage of what was discussed before as a "Bazaar" approach (as opposed to the "Cathedral" approach). Everything is available to everyone always, and updates are pushed very frequently (usually released to users in timed batches so as not to overwhelm them with constant updates). That being said, it definitely didn't begin there, and this concept is something that has existed for a very long time. Software has certainly made this process much easier, however. Many video games, for example, are now being released in an "Open Beta" state and are charging its players to play. Some of these games almost never leave Beta, taking years before the developer decides to "release" the game officially. This allows the game to add things fluidly without too much concern with the product potentially breaking, because it's technically in Beta, you signed up for it. This can be seen as a good or a bad thing, but the end result is this: The users get the product much earlier, and developers can receive funding much earlier as well. In comparison, video games back in the era of the Playstation 1, NES, etc did not popularly have internet connectivity capabilities. As such, game patches could never be pushed, and the game basically never changes after release. In that case, this sort of release ideology was almost impossible. Sometimes, a game would be released in one region first, then another region later. In that case, the copy of the game that was released later would actually tend to have bugs fixed, which is why when you look at the speedrunning community for video games, you'll see that people will prefer to play on certain regional releases of games, and they will be separated by region on leaderboards due to certain exploits only present in one regional release of a game. In that sense, this idea of releasing early and often wasn't possible, but now that it is, the overall gaming experience has changed in what can be considered a good way. However, it isn't quite on the level of what you see with something like Linux, as the ability to revert to an older version of a game is mostly nonexistent outside of somewhat illegitimate means.
